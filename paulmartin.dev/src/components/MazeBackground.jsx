...
// background. Dark at rest; it only reveals on mouse move (directional wake),
// scroll (wavy band), or ball impact. Controls are U/L/D/R (the letters hidden in
// "paulmartin.dev" — no arrow-key workaround). Reaching the exit calls onSolve.
//
// Ported verbatim from the standalone prototype (plans/maze-prototype.html) after a
// perf pass (spectrum-string memoisation); only the demo scaffolding was swapped for
// the onSolve callback and React lifecycle wiring.
export default function MazeBackground({ onSolve }) {
  ...
  useEffect(() => {
    ...
    const bands = [] // scroll reveal: wavy-edged rectangles {y0, dir, age, phase}
    ...
    // scroll wheel / two-finger trackpad emits a full-width band with wavy edges that
    // propagates in the scroll direction; debounced so a fast scroll fires one
    let lastRippleT = -1e9
    const onWheel = (e) => {
      if (nowT - lastRippleT < 800) return
      lastRippleT = nowT
      const dir = e.deltaY >= 0 ? 1 : -1
      bands.push({ y0: e.clientY, dir, age: 0, phase: Math.random() * 6.283 })
      if (bands.length > 6) bands.shift()
    }

    const ease = (t) => (t < 0.5 ? 4 * t * t * t : 1 - Math.pow(-2 * t + 2, 3) / 2)

    const WAKE_SPEED = 2.2, WAKE_LIFE = 52, RING_W = 38, REVEAL = 0.5
    const BAND_H = 66, BAND_SPEED = 4.4, BAND_LIFE = 50, BAND_AMP = 13
    ...
    const bandPath = (c2, cy, drift) => {
      const half = BAND_H / 2
      c2.beginPath()
      c2.moveTo(0, cy - half + BAND_AMP * Math.sin(drift))
      for (let x = 0; x <= W; x += 10) c2.lineTo(x, cy - half + BAND_AMP * Math.sin(x * 0.011 + drift))
      for (let x = W; x >= 0; x -= 10) c2.lineTo(x, cy + half + BAND_AMP * Math.sin(x * 0.013 - drift * 1.2))
      c2.closePath()
    }
    const warp = (c2, img) => {
      const strip = 6
    ...
    const renderReveal = (target, wantFull, includeBands, includeBallSelf, blackBack) => {
      ...
      if (includeBands) for (let i = 0; i < bands.length; i++) {
        const bd = bands[i], cy = bd.y0 + bd.dir * bd.age * BAND_SPEED, fade = 1 - bd.age / BAND_LIFE
        const drift = nowT * 0.002 + bd.phase, half = BAND_H / 2 + BAND_AMP
        bandPath(vctx, cy, drift)
        const g = vctx.createLinearGradient(0, cy - half, 0, cy + half)
        g.addColorStop(0, 'rgba(255,255,255,0)'); g.addColorStop(0.5, `rgba(255,255,255,${(0.9 * fade).toFixed(3)})`); g.addColorStop(1, 'rgba(255,255,255,0)')
      ...
      if (includeBands) for (let i = 0; i < bands.length; i++) {
        const bd = bands[i], cy = bd.y0 + bd.dir * bd.age * BAND_SPEED
        const drift = nowT * 0.002 + bd.phase
        const cg = mctx.createLinearGradient(0, 0, W, 0)
        const STOPS = 30, stops = specStops(STOPS, bd.age, BAND_LIFE)
        for (let s = 0; s <= STOPS; s++) cg.addColorStop(s / STOPS, stops[s])
        bandPath(mctx, cy, drift)
    ...
    function frame(now) {
      ...
      for (let i = bands.length - 1; i >= 0; i--) if (++bands[i].age > BAND_LIFE) bands.splice(i, 1)
      ...
      // BACK: ambient mouse wakes + scroll bands, revealed behind the content
      if (hasNonFull || bands.length) renderReveal(ctx, false, true, false, false)
